...
function sumTB_USD(rows, filterFn) {
  // Correct approach: for each account, compute monthly movement as
  // adjustedBalance(M) - adjustedBalance(M-1), then divide by FX(M).
  // This avoids the sign issues in balanceOfMonth (debit-credit).
  //
  // Group filtered rows by account, sort by month, then iterate:
  ...
  for (const acctRows of Object.values(byAcct)) {
    ...
    for (const r of acctRows) {
      ...
      total -= movement / fx;   // negate SAP sign (same as original sumTB)
...
function sumTBCredit_USD(rows, filterFn) {
  // Same approach: use adjustedCredit diff per month ÷ FX
  const filtered = rows.filter(filterFn);
  const byAcct = {};
  for (const r of filtered) {
    if (!byAcct[r.account]) byAcct[r.account] = [];
...
function computeMasriaIS_USD(tbISRows, opexRows, compCode, salesRows) {
  ...
  const sm = sumOpExpense_USD(opexRows, r =>
    keyIs(r, 6) && mappingIs(r, 'S&M') && r.companyCode === compCode && r.accountParent === MC_OPEX_PARENT
  );
  const ga = sumOpExpense_USD(opexRows, r =>
    keyIs(r, 6) && mappingIs(r, 'G&A') && r.companyCode === compCode && r.accountParent === MC_OPEX_PARENT
  );
...
function computeMdpIS_USD(tbISRows, opexRows, compCode, masriaSalesRows, dpSalesRows) {
  ...
  const sga = sumOpExpense_USD(opexRows, r =>
    keyIs(r, 6) && (mappingIs(r, 'S&M') || mappingIs(r, 'G&A')) &&
    r.accountParent === DP_OPEX_PARENT && r.companyCode === compCode
  );
  ...
  return {
    ...
    sga, ebitda, ebitdaM, grossDA, rouDA, totalDA,
