...
// ─────────────────────────────────────────────────────────────
// Aging/Sales data (used by the real DSO calc) is stored in USD natively,
// independent of the EGP/USD toggle — so these figures always show "$",
// never converted a second time.
function _usdAmt(v) { return '$' + FMT.compact(v); }
...
// Fallback DSO when no Aging snapshot exists for a given month: exact same
// methodology as the real DSO (_entityDSODetail) — Jan→month YTD revenue
// built from sales rows for this entity only, sister-company/intercompany
// codes excluded, then annualized via _calcDSO — just fed from the Balance
// Sheet's Receivables line instead of the Aging module's AR balance (since
// no Aging snapshot exists for this month). Revenue is converted to EGP
// (via salesValueEGP) rather than USD, to match Receivables' native TB
// currency — mixing USD revenue with EGP receivables would silently throw
// the result off by the FX rate.
function _fallbackReceivablesDays(ef, year, month, receivables) {
  if (!receivables) return null;
  if (typeof _getSalesRows !== 'function') return null;

  const entityCode = ef === 'mdp' ? 'dp' : 'mc';
  const folders     = entityCode === 'mc' ? ['modupay Cards', 'Masria Cards'] : ['modupay DP', 'mdp'];
  const opts        = entityCode === 'mc' ? { includeIntercompany: true } : {};
  const salesRows   = _getSalesRows(folders, year, month, opts).curr; // Jan→month, this entity only

  let ytdRevEGP = 0;
  for (const r of salesRows) {
    if (typeof AGING_DSO_EXCLUDED_SALES_CODES !== 'undefined' && AGING_DSO_EXCLUDED_SALES_CODES.has(r.code)) continue;
    ytdRevEGP += (typeof salesValueEGP === 'function') ? salesValueEGP(r) : (r.value || 0);
  }
  if (!ytdRevEGP) return null;
  ...
  return (typeof _calcDSO === 'function')
    ? _calcDSO(receivables, ytdRevEGP, daysYTD)
    : _days(receivables, ytdRevEGP);
}
...
// Pick the IS period sub-object in the currently-toggled currency —
// mirrors kpis.js's _getIS() exactly, just taking the IS object directly
// (this file already has IS in hand rather than a {IS,BS} statement).
function _isP(IS, period) {
  if (!IS) return {};
  if (STATE.usdMode) return IS[`_${period}USD`] || IS[period] || {};
  return IS[period] || {};
}

function _buildRatioSections(ef, IS, BS) {
  const { year, month } = STATE.filters;
  const PL = _extractPL(_isP(IS,'ytd'), _isP(IS,'sply'));
  const BSc = (STATE.usdMode && typeof _toBSUSD === 'function') ? _toBSUSD(BS, year, month) : BS;
  const B  = _extractBS(BSc);
  ...
  const receivablesEGP = STATE.usdMode ? _extractBS(BS).receivables : receivables; // fallback math stays in one consistent currency (EGP) — see _fallbackReceivablesDays
  const recDays   = dsoDetail?.dso ?? _fallbackReceivablesDays(ef, parseInt(year), parseInt(month), receivablesEGP);
  const recFormula = dsoDetail
    ? _fml('Receivables Days (DSO) = AR Balance ÷ (Annualized Revenue ÷ 365)',
        `${_usdAmt(dsoDetail.arBalance)} ÷ (${_usdAmt(dsoDetail.annualizedRevenue)} ÷ 365)`,
        _fd(dsoDetail.dso),
        `Annualized Revenue = YTD Revenue ${_usdAmt(dsoDetail.revenue)} × 365 ÷ ${Math.round(dsoDetail.daysYTD)} days covered · AR Aging data is USD-native, so this is unaffected by the EGP/USD toggle · same methodology as the AR Aging tab`)
    : _fml('Receivables Days = Receivables ÷ (YTD Revenue annualized ÷ 365)',
        `${_kpiAmt(receivables)} ÷ (Annualized YTD Revenue ÷ 365)`,
        _fd(recDays),
        'No matching Aging snapshot for this month — using the same YTD-annualized, intercompany-excluded revenue methodology as the real DSO, just against the Balance Sheet\'s Receivables line instead of the Aging AR balance');

  return [
    _sec('📈','Profitability',[
      _card('Gross Margin',   _fp(gpm),            'Gross Profit / Revenue',      _gauge(gpm),  _trend(gpm,lyGpm),   lyGpm!=null?`LY: ${_fp(lyGpm)}`:'',
        'grossMargin', ef, _fml('Gross Margin = Gross Profit ÷ Revenue', `${_kpiAmt(gp)} ÷ ${_kpiAmt(rev)}`, _fp(gpm))),
      _card('EBITDA Margin',  _fp(ebm),            'EBITDA / Revenue',            _gauge(ebm),  _trend(ebm,lyEbm),   lyEbm!=null?`LY: ${_fp(lyEbm)}`:'',
        'ebitdaMargin', ef, _fml('EBITDA Margin = EBITDA ÷ Revenue', `${_kpiAmt(ebitda)} ÷ ${_kpiAmt(rev)}`, _fp(ebm))),
      _card('EBIT Margin',    _fp(_p(ebit,rev)),   'EBIT / Revenue',              _gauge(_p(ebit,rev)), '', '',
        'ebitMargin', ef, _fml('EBIT Margin = EBIT ÷ Revenue', `${_kpiAmt(ebit)} ÷ ${_kpiAmt(rev)}`, _fp(_p(ebit,rev)), `EBIT = EBITDA ${_kpiAmt(ebitda)} + D&A ${_kpiAmt(da)}`)),
      _card('EBT Margin',     _fp(_p(ebt,rev)),    'EBT / Revenue',               _gauge(_p(ebt,rev)),  '', '',
        'ebtMargin', ef, _fml('EBT Margin = EBT ÷ Revenue', `${_kpiAmt(ebt)} ÷ ${_kpiAmt(rev)}`, _fp(_p(ebt,rev)))),
      _card('Net Margin',     _fp(netm),           'Net Profit / Revenue',        _gauge(netm), _trend(netm,lyNetm), lyNetm!=null?`LY: ${_fp(lyNetm)}`:'',
        'netMargin', ef, _fml('Net Margin = Net Profit ÷ Revenue', `${_kpiAmt(net)} ÷ ${_kpiAmt(rev)}`, _fp(netm))),
      _card('ROA',            _fp(_p(net,totalAssets||null)), 'Net / Total Assets',_gauge(_p(net,totalAssets||null)),'','',
        'roa', ef, _fml('ROA = Net Profit ÷ Total Assets', `${_kpiAmt(net)} ÷ ${_kpiAmt(totalAssets)}`, _fp(_p(net,totalAssets||null)))),
      _card('ROE',            _fp(_p(net,equity||null)),      'Net / Equity',      _gauge(_p(net,equity||null)),    '','',
        'roe', ef, _fml('ROE = Net Profit ÷ Equity', `${_kpiAmt(net)} ÷ ${_kpiAmt(equity)}`, _fp(_p(net,equity||null)))),
      _card('COGS Ratio',     _fp(_p(cogs,rev)),   'COGS / Revenue',              _gauge(_p(-cogs,rev),false),'','',
        'cogsRatio', ef, _fml('COGS Ratio = COGS ÷ Revenue', `${_kpiAmt(cogs)} ÷ ${_kpiAmt(rev)}`, _fp(_p(cogs,rev)))),
    ]),
    _sec('💧','Liquidity',[
      _card('Current Ratio', _fx(_x(currentAssets,currentLiab||null)),            'Current Assets / Current Liabilities','','', currentLiab?`CA ${_kpiAmt(currentAssets)} / CL ${_kpiAmt(currentLiab)}`:'',
        'currentRatio', ef, _fml('Current Ratio = Current Assets ÷ Current Liabilities', `${_kpiAmt(currentAssets)} ÷ ${_kpiAmt(currentLiab)}`, _fx(_x(currentAssets,currentLiab||null)))),
      _card('Quick Ratio',   _fx(_x(currentAssets-inventory,currentLiab||null)),  '(CA − Inventory) / Current Liabilities','','','',
        'quickRatio', ef, _fml('Quick Ratio = (Current Assets − Inventory) ÷ Current Liabilities', `(${_kpiAmt(currentAssets)} − ${_kpiAmt(inventory)}) ÷ ${_kpiAmt(currentLiab)}`, _fx(_x(currentAssets-inventory,currentLiab||null)))),
      _card('Cash Ratio',    _fx(_x(cash,currentLiab||null)),                     'Cash / Current Liabilities','','',cash?`Cash: ${_kpiAmt(cash)}`:'',
        'cashRatio', ef, _fml('Cash Ratio = Cash ÷ Current Liabilities', `${_kpiAmt(cash)} ÷ ${_kpiAmt(currentLiab)}`, _fx(_x(cash,currentLiab||null)))),
      _card('Working Capital',currentAssets&&currentLiab?_kpiAmt(currentAssets-currentLiab):'—','Current Assets − Current Liabilities','','','',
        'workingCapital', ef, _fml('Working Capital = Current Assets − Current Liabilities', `${_kpiAmt(currentAssets)} − ${_kpiAmt(currentLiab)}`, _kpiAmt(currentAssets-currentLiab))),
    ]),
    _sec('⚙️','Efficiency',[
      _card('Asset Turnover',      _fx(_x(rev,totalAssets||null)),    'Revenue / Total Assets','','','',
        'assetTurnover', ef, _fml('Asset Turnover = Revenue ÷ Total Assets', `${_kpiAmt(rev)} ÷ ${_kpiAmt(totalAssets)}`, _fx(_x(rev,totalAssets||null)))),
      _card('Receivables Days',    _fd(recDays), dsoDetail?'AR Balance ÷ Annualized Revenue × 365 (DSO)':'Receivables / Revenue × 365','','',receivables?`AR: ${_kpiAmt(receivables)}`:'',
        'receivablesDays', ef, recFormula),
      _card('Fixed Asset Turnover',_fx(_x(rev,nonCurrAssets||null)), 'Revenue / Non-Current Assets','','','',
        'fixedAssetTurnover', ef, _fml('Fixed Asset Turnover = Revenue ÷ Non-Current Assets', `${_kpiAmt(rev)} ÷ ${_kpiAmt(nonCurrAssets)}`, _fx(_x(rev,nonCurrAssets||null)))),
    ]),
    _sec('🏦','Leverage',[
      _card('Debt / Equity',  equity?_fx(_x(totalLiab,equity)):'—',      'Total Liabilities / Equity','','','',
        'debtEquity', ef, _fml('Debt / Equity = Total Liabilities ÷ Equity', `${_kpiAmt(totalLiab)} ÷ ${_kpiAmt(equity)}`, equity?_fx(_x(totalLiab,equity)):'—')),
      _card('Debt / Assets',  _fx(_x(totalLiab,totalAssets||null)),       'Total Liabilities / Total Assets',_gauge(_p(-totalLiab,totalAssets||null),false),'','',
        'debtAssets', ef, _fml('Debt / Assets = Total Liabilities ÷ Total Assets', `${_kpiAmt(totalLiab)} ÷ ${_kpiAmt(totalAssets)}`, _fx(_x(totalLiab,totalAssets||null)))),
      _card('Equity Ratio',   _fx(_x(equity,totalAssets||null)),          'Equity / Total Assets',_gauge(_p(equity,totalAssets||null)),'','',
        'equityRatio', ef, _fml('Equity Ratio = Equity ÷ Total Assets', `${_kpiAmt(equity)} ÷ ${_kpiAmt(totalAssets)}`, _fx(_x(equity,totalAssets||null)))),
      _card('Interest Cover', finExp>0?_fx(_x(ebit,finExp)):'—',         'EBIT / Financial Expenses','','','',
        'interestCover', ef, _fml('Interest Cover = EBIT ÷ Financial Expenses', `${_kpiAmt(ebit)} ÷ ${_kpiAmt(finExp)}`, finExp>0?_fx(_x(ebit,finExp)):'—')),
    ]),
    _sec('🚀','Growth vs Prior Year',[
      _card('Revenue Growth',    lyRev?_fp(_p(rev-lyRev,lyRev)):'—',      'YTD Revenue vs SPLY',        _gauge(lyRev?_p(rev-lyRev,lyRev):null), '','LY: '+_kpiAmt(lyRev),
        'revenueGrowth', ef, _fml('Revenue Growth = (Revenue − LY Revenue) ÷ LY Revenue', `(${_kpiAmt(rev)} − ${_kpiAmt(lyRev)}) ÷ ${_kpiAmt(lyRev)}`, lyRev?_fp(_p(rev-lyRev,lyRev)):'—')),
      _card('GP Growth',         lyGP?_fp(_p(gp-lyGP,lyGP)):'—',         'YTD Gross Profit vs SPLY',   _gauge(lyGP?_p(gp-lyGP,lyGP):null),    '','LY: '+_kpiAmt(lyGP),
        'gpGrowth', ef, _fml('GP Growth = (Gross Profit − LY Gross Profit) ÷ LY Gross Profit', `(${_kpiAmt(gp)} − ${_kpiAmt(lyGP)}) ÷ ${_kpiAmt(lyGP)}`, lyGP?_fp(_p(gp-lyGP,lyGP)):'—')),
      _card('EBITDA Growth',     lyEb?_fp(_p(ebitda-lyEb,lyEb)):'—',     'YTD EBITDA vs SPLY',         _gauge(lyEb?_p(ebitda-lyEb,lyEb):null),'','',
        'ebitdaGrowth', ef, _fml('EBITDA Growth = (EBITDA − LY EBITDA) ÷ LY EBITDA', `(${_kpiAmt(ebitda)} − ${_kpiAmt(lyEb)}) ÷ ${_kpiAmt(lyEb)}`, lyEb?_fp(_p(ebitda-lyEb,lyEb)):'—')),
      _card('Net Profit Growth', lyNet?_fp(_p(net-lyNet,lyNet)):'—',      'YTD Net Profit vs SPLY',     _gauge(lyNet?_p(net-lyNet,lyNet):null), '','',
        'netProfitGrowth', ef, _fml('Net Profit Growth = (Net Profit − LY Net Profit) ÷ LY Net Profit', `(${_kpiAmt(net)} − ${_kpiAmt(lyNet)}) ÷ ${_kpiAmt(lyNet)}`, lyNet?_fp(_p(net-lyNet,lyNet)):'—')),
...
function _renderEntityRatios(ef, name, IS, BS) {
  ...
  const lyRev=_isP(IS,'sply').revenue||0, lyNet=_isP(IS,'sply').netProfit||0;
...
function _ratioMonthMetrics(ef, ym) {
  ...
  const BSmRaw = buildEntityBSForEntity(ef, ym.year, ym.month);
  const BSm = (STATE.usdMode && typeof _toBSUSD === 'function') ? _toBSUSD(BSmRaw, ym.year, ym.month) : BSmRaw;
  const PL  = _extractPL(P, PLY);
  const B   = _extractBS(BSm);
  const dso = _entityDSO(ef, ym.year, ym.month);
  const receivablesEGP = STATE.usdMode ? _extractBS(BSmRaw).receivables : B.receivables;
  ...
  return {
    ...
    receivablesDays:     dso!=null ? dso : _fallbackReceivablesDays(ef, ym.year, ym.month, receivablesEGP),
